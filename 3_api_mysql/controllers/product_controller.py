...
async def obtener_producto_by_id(product_id):
    try:
...
#obtener productos entre un rango de precioes

async def obtener_productos_by_price(pricemin,pricemax):
...
async def actualizar_producto_by_id(product_id, product: product):
    ...
    try:     
        ...
        async with conn.cursor(aio.DictCursor) as cursor:
            #se lanza la consulta
            await cursor.execute("UPDATE products SET name=%s, surname=%s, price=%s, mail=%s, status=%s, password=%s, rol=%s WHERE id=%s", ( product.name, product.surname, product.price, product.mail, product.status, product.password, product.rol,product.id))
